...
This script is at base a copy of clean_gator_output.py

In essence, we should be able to see what the distribution of objects
affected by source confusion is, but it's not guaranteed.
...
We want sources with CC Flags and NOT the reliable ones, so data[jj][-9] != '0000'.
We still want to exclude extended sources, so data[jj][-8] == 0
We still want 2MASS photometry, but I'm relaxing the matches, so data[jj][-7] >= 1;
Because we're letting all of this extra stuff in, I'm going to keep
these 3 columns in the output data.

Outputting the results to the file
	agbcandidates_ALLWISE_unreliable.dat
...
nobj = len(snrcut[0])
print '%i unreliable objects' % nobj
...
ccflags = ccflags[snrcut]
extflag = extflag[snrcut]
n_2mass = n_2mass[snrcut]
j = j[snrcut]
jerr = jerr[snrcut]
h = h[snrcut]
herr = herr[snrcut]
...
outfile_dir = '/Users/Nick/Documents/AGBstuff/workstation/allwise_reboot/unreliable_candidates/'

fout = open(outfile_dir+'agbcandidates_ALLWISE_unreliable.dat','w')
fmt = '%s,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%s,%g,%g,%g,%g,%g,%g,%g,%g\n'

fout.write('wid,ra,dec,gall,galb,w1,w2,w3,w4,w1err,w2err,w3err,w4err,w1snr,w2snr,w3snr,w4snr,pmra,pmdec,sigpmra,sigpmdec,ccflags,extflag,n_2mass,j,h,k,jerr,herr,kerr\n')
for ii in range(nobj):
	fout.write(fmt % (wid[ii],ra[ii],dec[ii],gall[ii],galb[ii],w1[ii],w2[ii],w3[ii],w4[ii],w1err[ii],w2err[ii],w3err[ii],w4err[ii],w1snr[ii],w2snr[ii],w3snr[ii],w4snr[ii],pmra[ii],pmdec[ii],sigpmra[ii],sigpmdec[ii],ccflags[ii],extflag[ii],n_2mass[ii],j[ii],h[ii],k[ii],jerr[ii],herr[ii],kerr[ii]))
fout.close()
